...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec  3 10:53:14 2019
...
def sumRows(trans, a):
    for i in range(a):
        print(np.sum(trans[i], axis = 1))

def policyEvaluation(trans, rewards, n, a, gamma, policy):
    I = np.identity(n)
    p_pi = []
    for i in range(n):
        p_pi.append(trans[policy[i]][i])
    p_pi = np.array(p_pi)
    value = np.matmul(np.linalg.inv(I - gamma * p_pi), rewards)
    return value

def policyImprovement(trans, rewards, n, a, gamma, policy, value):
    p = []
    for s in range(n):
        arr = []
        for i in range(a):
...
def policyIteration(trans, rewards, n, a, gamma, iteration):
    policy = np.zeros(n, dtype = 'int')
    for i in range(iteration):
        value = policyEvaluation(trans, rewards, n, a, gamma, policy)
        policy = policyImprovement(trans, rewards, n, a, gamma, policy, value)
    r = int(math.sqrt(n))
...
def printResult(value, policy, n):
    np.savetxt('value_policy.out', value, delimiter=' ')
    np.savetxt('policy_policy.out', policy, delimiter=' ')
    string = []
    r = int(math.sqrt(n))
    for i in range(r):
        s = []
        for j in range(r):
            if value[i][j] == 0:
                ...
                if policy[i][j] == 0:
                    ...
                    s.append('v ')
        string.append(s)
    with open('action_policy.out', 'w') as file:
...
if __name__ == "__main__":
    ...
    #sumRows(trans, a)
    value, policy = policyIteration(trans, rewards, n, a, gamma, iteration)
